...
func newOrchestratorOnlyNode(t *testing.T) *orchestratorOnlyNode {
	...
	copyManagedBinary(t, enforcerBin, orchestrator.BinaryPath(bitwindowDir, "lip005-enforcer"))
...
func findEnforcer(t *testing.T) string {
	...
	orchPath := orchestrator.BinaryPath(dataDir, "lip005-enforcer")

	if _, err := os.Stat(orchPath); err == nil {
		log.Info().Str("path", orchPath).Msg("found lip005-enforcer binary")
		return orchPath
	}
