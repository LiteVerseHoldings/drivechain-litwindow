...
We show how to use Eth's software stack, but with LTC. (Ie, without buying Ether). This requires LIP 005, but we can still demonstrate via DriveNet (a version of Litecoin Core with LIP 005 activated).
...
4. Use the third terminal window to play with ethereum – make accounts, use Eth functions, and especially: deposit 10 LTC from the mainchain to the Eth Sidechain.
...
That command will take 10 of your LTC from the mainchain (since you've been mining this whole time), and sends them over to the ETH sidechain.
...
We ran an ETH contract… on top of LTC. We didn't buy any Ether! And we left Layer1 alone – our layer1 Litecoin mainchain full node, knew nothing of Ethereum's rules.

This guide didn't cover withdrawals (ie: when coins move from sidechain to mainchain). But that's only because LIP 005 withdrawals have already been covered to death.
