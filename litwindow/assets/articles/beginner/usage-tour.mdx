...
- Send LTC from the mainchain to a sidechain.
- Use the sidechain (send LTC from ourselves to ourselves).
- Send the sidechain-LTC back to the mainchain.
...
### B. Drivenet-LTC vs Drivenet-Blank
...
It's the July 30th UTXO set from the LTC network at [block 534,444](https://www.blockchain.com/ltc/block/00000000000000000012ff6b14f186d7b7c41d8e585a40d79f1164dcd92f8b36). Uncompressed, it is 4.8 GB.
...
In the near future, we plan to release a "Drivenet-BCH" that has the Litecoin Cash UTXO set.
...
To start up 'Sidechain One', go into the sidechain folder ("/litecoin-0.16.99/" in these images, in later versions the folder is named "/sidechain-N-0.16.99/"). And enter the /bin/ directory.
...
### D. Sending LTC Main-to-Side

Below, I have arranged the mainchain on the left (in blue) and the sidechain "Sidechain One" on the right (in red).

#### i. Getting drivenet-LTC

First, in order to send some LTC, you'll need some LTC to send!
...
- NOT RECOMMENDED claiming a UTXO that you own on the LTC network by importing your private key ( Help > Debug window > Console > "import <KEY>") NOT RECOMMENDED

The last method is not recommended, because *you should never type your private key into software, unless you've checked that software very carefully*. Which, at this point, you have not. Furthermore, if you misunderstand how we have implemented replay protection, you may destroy your regular (non-drivenet) LTC funds.

Don't worry about asking me for LTC. For testing purposes, CryptAxe and I modded the UTXO set to give ourselves 100,000 each. We're loaded! Plus these are useless testnet coins.
...
In this guide, I will assume you have asked for some LTC. You'd do this, of course, by using the "Receive" tab > "Request Payment" > "Copy Address".

And then I assume you've received 21 LTC.
...
It should show up, as "Immature". Since the coins were created out of nothing, the software classifies these LTC as "mined". We are going to change that to something drivechain-specific soon.

### E. Using the Sidechain

We've now got 15.9999,0880 LTC on "Sidechain One".

Let's send it to ourselves, to simulate the act of paying a merchant for a good or service.

Get a new address.

Send 11 LTC (of our 15.9999,0880) to it.

Click "Send", our balance decreases very slightly, to 15.9998,6380, as we paid a transaction fee. See the bottom right corner.

Using the "Coin Control" > "Inputs" button, we can see the new 11 LTC UTXO that we paid to the "merchant" (aka ourselves).

### F. Sending LTC Side-to-Main

Now, let us imagine that our hypothetical merchant wants to move his LTC from the sidechain back to the mainchain.
...
Looks like I've been reading litecoin-dev long enough, we now have 143 ACKs. The "Approved" field has switched to "true". This WT^ container transaction can now be included in a mainchain block.
...
- 5 LTC – Change from when we sent 16 of our 21 starting LTC to the sidechain.
- 9.98996640 LTC – the 10 LTC side-to-main transfer, minus various sidechain and mainchain fees.

## Conclusion

The tour is complete! We demonstrated sending LTC to a different piece of software. Then we sent that LTC back to the mainchain.
