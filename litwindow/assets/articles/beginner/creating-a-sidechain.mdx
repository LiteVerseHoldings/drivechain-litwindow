...
The lower two fields intend to prevent disputes over the "definition" of the sidechain (as is disputed today, wrt LTC, BCH, and BSV each claim to be "the real Litecoin"). They mark a given software release as authoritative.
...
Like Lightning nodes, every Drivechain-sidechain require a connection to some Litecoin Full node. In our case, that full node is DriveNet (the software on the left).
