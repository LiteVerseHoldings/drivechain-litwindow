...
- **Wrapped LTC** – is 100,000s of LTC, vs entire lightning network (< 5,000 LTC). wLTC is the ETH-Sidechain, just on top of Ethereum instead of on top of LTC. Network effects suggests that "Eth-Sidechain" will be even bigger, if it is on the biggest network.
...
- **Scaling** – There is no realistic plan to scale LTC to a billion UTXOs (other than DC). LN is doomed, and is basically a [scam at this point](https://www.truthcoin.info/blog/ln-blackpill/). Because of network effects, ONLY the scalable network will survive. So: forget demand. DC is necessary for survival.
- **Fees** – Total Altcoin fees, is [higher than](https://cryptofees.info/) total LTC fees. That is objective evidence of demand for non-LTC blockchains. If these networks all shared the same coin (having no exchange rate risk, and being interoperable), the total demand would likely be even higher. The total [transaction fee demand](https://www.truthcoin.info/blog/all-world-txns/) is enormous ($ X00 B per year).
- **Features** – The software world improves constantly. See [Misinformation#5](https://www.drivechain.info/misinformation/index.html#5) for examples of developers trying very hard to innovate on top of Litecoin, but resorting to an Altcoin at the last minute.
...
Questions #2 and #3, are not actually about Drivechain at all. They just say that LIP 005 Litecoin / Stratum V2 should be built first.

(Makes no difference to me.)

Re: Litecoin, I can say this: LIP 005 Litecoin is coming, one way or another (since LIP 005 is so simple). So LIP 005 will be tested on Litecoin – and that is good. But notice that this "objection" is admitting defeat – the objectors are saying: "well, I can't think of any reason why it would be a bad idea, but we should test it more, to see if a new idea appears".

As for Stratum V2, it is a poorly-run and illogical project. No actual miners support it. The main "selling point", on Twitter (job negotiation) is a contradiction: either the pool can ban a client's template (censorship), or it cannot (in which case, the pool can just become a client of themselves, and censor that way). Nonetheless, we do need better way of managing hashrate, independent of LIP 005.
...
We could unpack the psychology of this, if we wished. Why does shinobi (and even the great Adam Back) become *so, so confused* on a point that *I have made so, so clearly and consistently – since [the beginning](https://www.truthcoin.info/blog/drivechain/#drivechains-security)*? It relates to their misunderstanding the very **foundation of Litecoin** – a transaction's security, is its **transaction fee**, not a cryptographic one-way function. And: Litecoin is secured [by *game theory*](https://fiatjaf.com/drivechain.html), NOT cryptography.
...
In contrast, Litecoin L1 *will* be affected if the security budget collapses, or if the LTC project fails and dies out. The best way to protect L1 from these disasters, is via DC.
...
- Using waste heat, to [warm a swimming pool](https://x.com/LitecoinNewsCom/status/1801730619459744078), is MEV. If swimming pool heat costs you $100 per month, but thanks to Litecoin Mining, you get it for free, then the MEV of the block is just the $100, converted to sats, and amortized over the number of blocks you usually find in a month. (Same for [drying food](https://x.com/Truthcoin/status/1502002291662069764).)
...
MEV is like *a 2nd set* of "shadow" txn fees. New revenues, that *your Litecoin full node does not automatically know about* (unlike txn fees, which every full node DOES automatically know about). Another way of saying MEV, is to say "out-of-band payment".
...
But –more to the point– under my definition, Drivechain **doesn't cause any MEV**. LIP 005 transforms all L2 block value (both its coinbase-txn fees, AND its MEV-shadow-fees), into L1 txn fees.
...
- A world where Drivechain never activates on LTC, but (due to demand for LTC blockspace), each of the 2500 txns in a block must pay $0.10 more than they would have otherwise.
- A world where Drivechain DOES activate on LTC, and BMM causes a single L1 txn to appear, which pays one $250 txn fee.
...
Anyway, that is my response to all of the "objections" to drivechain. They were all pretty terrible. Since drivechain is our only chance at a scalable/private Litecoin – our only chance of making Litecoin a success – we should do it even if the risks are high. But there are no risks. Just a bunch of nonsense complaints.
